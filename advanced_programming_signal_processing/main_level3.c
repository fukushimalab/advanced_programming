...
void templateMatchingColor(Image *src, Image *templ, Point *position, double *distance)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int y = 0; y < src->height - templ->height; y++)
        {
            ...
            {
                if (isMatchColor(src, templ, x, y, alpha[i]))
                {
                    position->x = x;
                    position->y = y;
                    *distance = 0;
...
int isMatchColor(Image *src, Image *templ, int x, int y, double alpha)
{
    for (int j = 0; j < templ->height; j++)
    {
        for (int i = 0; i < templ->width; i++)
...
void changeAlpha(Pixel *p, double alpha)
{
